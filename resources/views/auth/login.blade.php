...
<div class="flex h-screen flex-col md:flex-row">
    ...
    <div class="flex-1 text-black p-8 flex flex-col justify-center items-center bg-gray-200">
        ...
        <form method="POST" action="{{ route('login') }}" class="w-full max-w-md mt-8 space-y-6">
            @csrf
            <!-- Email -->
            <input id="email" type="email" name="email" value="{{ old('email') }}" requiblue autocomplete="email" placeholder="Email"
        ...
        <div class="mt-4 text-sm text-black">
            <p>Don't have an account? <a href="/register" class="text-blue-600 hover:underline font-bold">Sign Up</a>
            </p>
        </div>
    </div>
</div>
