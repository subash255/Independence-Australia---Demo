        ...
        @foreach ($reviews as $review)
            @if ($review->status == 1)
                <div x-data="{ open: false }"
                    class="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 min-h-[250px]">
    ...
    <div id="authModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white p-6 rounded-lg w-96 text-center">
            ...
            <div class="flex justify-between mt-6 space-x-4">
                ...
                <button id="closeAuthModalButton"
                    class="w-full md:w-auto bg-red-500 text-white py-2 px-4 font-semibold rounded-lg hover:bg-red-600 transition duration-300 focus:outline-none">
                    Cancel
                </button>
                ...
                <a href="{{ route('login') }}"
                    class="w-full md:w-auto bg-gradient-to-r from-indigo-600 to-indigo-700 text-white font-semibold py-2 px-4 rounded-lg hover:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-300 transform hover:scale-105">
                    Log In
                </a>
            </div>
        </div>
        ...
        stars.forEach(star => {
            ...
            star.addEventListener('mouseout', function() {
                stars.forEach(star => {
                    ...
                    'text-yellow-300'); // Remove light yellow when mouse is out
                });
            });
        });
    </script>
